...
const STATS = [
  ...
  { value: 'Any', label: 'language, genre, sub-genre, and instrument family' },
...
export function Hero() {
  ...
  return (
    <section style={{ padding: 'clamp(120px, 14vw, 180px) 32px clamp(80px, 10vw, 120px)', position: 'relative', overflow: 'hidden', backgroundImage: "url('/brand/aurora-grid.svg')", backgroundSize: 'cover', backgroundPosition: 'center' }}>
      <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(180deg, rgba(7,8,10,0.2) 0%, rgba(7,8,10,0.78) 100%)', pointerEvents: 'none' }} />
      ...
      <div style={{ maxWidth: '980px', margin: '0 auto', textAlign: 'center', position: 'relative' }}>
        ...
        <p className="hs3" style={{ fontSize: 'clamp(16px, 2vw, 20px)', color: 'var(--text-secondary)', lineHeight: 1.75, maxWidth: '720px', margin: '0 auto 44px', letterSpacing: '-0.01em' }}>
          ARE-E is a production intelligence system built for every genre and sub-genre, any language, and any ensemble format. From chart pop to Hans Zimmer-style cinematic concepts, it maps creative intent into lyrics, structure, instrumentation direction, and generation-ready execution prompts in one run.
        </p>
